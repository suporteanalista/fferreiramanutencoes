/*
# Fix restore_backup: delete in reverse dependency order

1. Problem
   - The previous version deleted tables in forward dependency order (parent before child).
   - This fails on re-run because existing child rows (e.g. ordens_servico) reference
     parent rows (e.g. clientes), blocking the DELETE.

2. Fix
   - Delete ALL tables first, in REVERSE dependency order (children before parents):
     os_produtos, ordens_servico, equipamentos, produtos, tecnicos, clientes, configuracoes
   - Then INSERT all tables in FORWARD dependency order (parents before children):
     configuracoes, clientes, tecnicos, produtos, equipamentos, ordens_servico, os_produtos
   - This ensures foreign keys are respected on both delete and insert.
...
BEGIN
    ...
    -- ═══════════════════════════════════════════════════
    -- PHASE 1: DELETE all tables in REVERSE dependency order
    ...
    -- ═══════════════════════════════════════════════════
    -- PHASE 2: INSERT all tables in FORWARD dependency order
    ...
    FOR v_record IN SELECT * FROM jsonb_array_elements(v_records) LOOP
        ...
        EXCEPTION WHEN others THEN
            v_failed := v_failed + 1;
            v_errors := array_append(v_errors, 'configuracoes: ' || SQLERRM);
        END;
    END LOOP;
    ...
    FOR v_record IN SELECT * FROM jsonb_array_elements(v_records) LOOP
        ...
        EXCEPTION WHEN others THEN
            v_failed := v_failed + 1;
            v_errors := array_append(v_errors, 'clientes: ' || SQLERRM);
        END;
    END LOOP;
    ...
    FOR v_record IN SELECT * FROM jsonb_array_elements(v_records) LOOP
        ...
        EXCEPTION WHEN others THEN
            v_failed := v_failed + 1;
            v_errors := array_append(v_errors, 'tecnicos: ' || SQLERRM);
        END;
    END LOOP;
    ...
    FOR v_record IN SELECT * FROM jsonb_array_elements(v_records) LOOP
        ...
        EXCEPTION WHEN others THEN
            v_failed := v_failed + 1;
            v_errors := array_append(v_errors, 'produtos: ' || SQLERRM);
        END;
    END LOOP;
    ...
    FOR v_record IN SELECT * FROM jsonb_array_elements(v_records) LOOP
        ...
        EXCEPTION WHEN others THEN
            v_failed := v_failed + 1;
            v_errors := array_append(v_errors, 'equipamentos: ' || SQLERRM);
        END;
    END LOOP;
    ...
    FOR v_record IN SELECT * FROM jsonb_array_elements(v_records) LOOP
        ...
        EXCEPTION WHEN others THEN
            v_failed := v_failed + 1;
            v_errors := array_append(v_errors, 'ordens_servico: ' || SQLERRM);
        END;
    END LOOP;
    ...
    FOR v_record IN SELECT * FROM jsonb_array_elements(v_records) LOOP
        ...
        EXCEPTION WHEN others THEN
            v_failed := v_failed + 1;
            v_errors := array_append(v_errors, 'os_produtos: ' || SQLERRM);
        END;
    END LOOP;
