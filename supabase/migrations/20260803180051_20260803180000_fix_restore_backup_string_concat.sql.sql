/*
# Fix restore_backup: extract jsonb values into text vars before concatenation

1. Problem
   - Using v_record->>'field' directly inside string concatenation with || 
   - PostgreSQL's parser sometimes interprets ->> as a JSON operator in the
     concatenation context, causing "operator does not exist: text ->> unknown".
   - Also, array_append to text[] with a string containing ->> causes JSON parsing errors.

2. Fix
   - Extract all jsonb field values into local text variables before using
     them in string concatenation or array_append calls.
   - Only the ordens_servico validation block was affected; all other blocks
     already used the v_rec_id variable pattern correctly.
*/
        ...
        IF v_fk_uuid IS NOT NULL THEN
            IF NOT (v_fk_uuid = ANY(v_db_profiles_ids) OR EXISTS (SELECT 1 FROM profiles WHERE id = v_fk_uuid)) THEN
                ...
                v_validation_errors := array_append(v_validation_errors,
                    'ordens_servico[' || v_i || '] (id=' || v_rec_id || '): criado_por FK invalido: ' || v_fk_text ||
