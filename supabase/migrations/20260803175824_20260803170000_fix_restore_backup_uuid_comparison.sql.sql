/*
# Fix restore_backup: correct UUID type comparisons in FK validation

1. Problem
   - The FK validation code compared text values directly with uuid arrays
     using `v_fk = ANY(v_clientes_ids)` where v_fk is text and the array is uuid[].
   - PostgreSQL raises "operator does not exist: text = uuid".

2. Fix
   - Cast text values to uuid before comparing with uuid arrays.
   - All FK validation blocks updated consistently.
*/
...
BEGIN
    ...
    FOR v_i IN 0..jsonb_array_length(v_data->'equipamentos')-1 LOOP
        ...
        ELSIF NOT (v_fk_uuid = ANY(v_clientes_ids) OR v_fk_uuid = ANY(v_db_clientes_ids) OR EXISTS (SELECT 1 FROM clientes WHERE id = v_fk_uuid)) THEN
            v_validation_errors := array_append(v_validation_errors, 'equipamentos[' || v_i || ']: cliente_id FK invalido: ' || v_record->>'cliente_id' || ' nao existe em clientes');
        END IF;
    ...
    FOR v_i IN 0..jsonb_array_length(v_data->'ordens_servico')-1 LOOP
        ...
        ELSIF NOT (v_fk_uuid = ANY(v_clientes_ids) OR v_fk_uuid = ANY(v_db_clientes_ids) OR EXISTS (SELECT 1 FROM clientes WHERE id = v_fk_uuid)) THEN
            v_validation_errors := array_append(v_validation_errors, 'ordens_servico[' || v_i || ']: cliente_id FK invalido: ' || v_record->>'cliente_id' || ' nao existe em clientes');
        END IF;
        ...
        IF v_fk_uuid IS NOT NULL THEN
            IF NOT (v_fk_uuid = ANY(v_equipamentos_ids) OR v_fk_uuid = ANY(v_db_equipamentos_ids) OR EXISTS (SELECT 1 FROM equipamentos WHERE id = v_fk_uuid)) THEN
                v_validation_errors := array_append(v_validation_errors, 'ordens_servico[' || v_i || ']: equipamento_id FK invalido: ' || v_record->>'equipamento_id' || ' nao existe em equipamentos');
            END IF;
        END IF;
        ...
        IF v_fk_uuid IS NOT NULL THEN
            IF NOT (v_fk_uuid = ANY(v_tecnicos_ids) OR v_fk_uuid = ANY(v_db_tecnicos_ids) OR EXISTS (SELECT 1 FROM tecnicos WHERE id = v_fk_uuid)) THEN
                v_validation_errors := array_append(v_validation_errors, 'ordens_servico[' || v_i || ']: tecnico_id FK invalido: ' || v_record->>'tecnico_id' || ' nao existe em tecnicos');
            END IF;
        END IF;
        -- criado_por (nullable FK -> profiles)
        v_fk_uuid := NULLIF(v_record->>'criado_por', '')::uuid;
        IF v_fk_uuid IS NOT NULL THEN
            IF NOT (v_fk_uuid = ANY(v_db_profiles_ids) OR EXISTS (SELECT 1 FROM profiles WHERE id = v_fk_uuid)) THEN
                v_validation_errors := array_append(v_validation_errors,
                    'ordens_servico[' || v_i || '] (id=' || v_rec_id || '): criado_por FK invalido: ' || v_record->>'criado_por' ||
        ...
        END IF;
        -- status CHECK
        IF v_record->>'status' IS NOT NULL AND v_record->>'status' NOT IN ('aberta','em_andamento','aguardando_peca','concluida','entregue') THEN
            v_validation_errors := array_append(v_validation_errors, 'ordens_servico[' || v_i || ']: status invalido: ' || v_record->>'status');
        END IF;
        -- prioridade CHECK
        IF v_record->>'prioridade' IS NOT NULL AND v_record->>'prioridade' NOT IN ('baixa','normal','alta','urgente') THEN
            v_validation_errors := array_append(v_validation_errors, 'ordens_servico[' || v_i || ']: prioridade invalida: ' || v_record->>'prioridade');
        END IF;
    END LOOP;
    ...
    FOR v_i IN 0..jsonb_array_length(v_data->'os_produtos')-1 LOOP
        ...
        ELSIF NOT (v_fk_uuid = ANY(v_ordens_ids) OR EXISTS (SELECT 1 FROM ordens_servico WHERE id = v_fk_uuid)) THEN
            v_validation_errors := array_append(v_validation_errors, 'os_produtos[' || v_i || ']: ordem_servico_id FK invalido: ' || v_record->>'ordem_servico_id' || ' nao existe em ordens_servico');
        END IF;
        ...
        ELSIF NOT (v_fk_uuid = ANY(v_produtos_ids) OR v_fk_uuid = ANY(v_db_produtos_ids) OR EXISTS (SELECT 1 FROM produtos WHERE id = v_fk_uuid)) THEN
            v_validation_errors := array_append(v_validation_errors, 'os_produtos[' || v_i || ']: produto_id FK invalido: ' || v_record->>'produto_id' || ' nao existe em produtos');
        END IF;
    END LOOP;
